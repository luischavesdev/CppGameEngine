...
ShaderProgram::ShaderProgram(GameEngine* engineRef)
{
	...
	float tempVertices[] = {// aPos x, y,     TexCoord x, y
...
void ShaderProgram::Start(bool defaultStart, float windowAspect)
{
	...
	if (!success)
	{
		...
		CUSTOM_LOGGER("ERROR: SHADER::VERTEX::COMPILATION_FAILED");
	}
	...
	if (!success)
	{
		...
		CUSTOM_LOGGER("ERROR: SHADER::FRAGMENT::COMPILATION_FAILED");
	}
	...
	if (!success) {
		...
		CUSTOM_LOGGER("ERROR: SHADER::PROGRAM::COMPILATION_FAILED");
	}
